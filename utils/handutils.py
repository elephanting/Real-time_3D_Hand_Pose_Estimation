...
def get_mask_mini_scale(mask_, side):
    ...
    if side == "l":
        ...
        xywh[1] = (seg_rmin + seg_rmax) / 2 / w
        xywh[0] = (seg_cmin + seg_cmax) / 2 / h
        xywh[3] = (seg_rmax - seg_rmin) / w
        xywh[2] = (seg_cmax - seg_cmin) / h
        ...
        xywh[1] = (rmin + rmax) / 2 / w
        xywh[0] = (cmin + cmax) / 2 / h
        xywh[3] = (rmax - rmin) / w
        xywh[2] = (cmax - cmin) / h
    ...
    return mask_scale, xywh


def get_kp2d_mini_scale(annots, h, w):
    """
    ...
    max_x, max_y = annots.max(0)
    xywh[0] = (min_x + max_x) / 2 / w
    xywh[1] = (min_y + max_y) / 2 / h
    xywh[2] = (max_x - min_x) / w
    xywh[3] = (max_y - min_y) / h
...
def get_ori_crop_scale(mask, side, kp2d, mask_flag=True,scale_factor=1.0, h=None, w=None):
    kp2d_mini_scale, xywh = get_kp2d_mini_scale(kp2d, h, w)
    ...
    if mask_flag:
        ...
        mask_mini_scale, xywh = get_mask_mini_scale(mask, side)
        ori_crop_scale = max(mask_mini_scale, kp2d_mini_scale)
    ...
    return ori_crop_scale * scale_factor, xywh
...
def rot_box(targets, rot2d):
    '''
    targets: cx, cy, w, h
    '''
    targets = xywh2xyxy(targets[None, ...])
    xy = np.ones((4, 3))
    xy[:, :2] = targets[:, [0, 1, 2, 3, 0, 3, 2, 1]].reshape(4, 2)  # x1y1, x2y2, x1y2, x2y1
    xy = (xy @ rot2d.T)[:, :2].reshape(1, 8)

    # create new boxes
    x = xy[:, [0, 2, 4, 6]]
    y = xy[:, [1, 3, 5, 7]]
    xy = np.concatenate((x.min(1), y.min(1), x.max(1), y.max(1))).reshape(4, 1).T
    return xyxy2xywh(xy).squeeze(0)
