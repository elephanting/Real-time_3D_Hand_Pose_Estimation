...
def xyz_to_delta_tensor(xyz, joints_def, device='cpu'):
    ...
    for j in range(joints_def.n_joints):
        p = joints_def.parents[j]
        if p is None:
            delta.append(torch.zeros(b, 3).to(device))
        else:
            delta.append(xyz[:, j] - xyz[:, p])
    delta = torch.cat(delta, dim=1)
    delta = rearrange(delta,'b (j c)->b j c',c = 3)

    lengths = torch.norm(delta, dim =-1, keepdim=True)
    eps = torch.tensor(np.finfo(float).eps).to(device).type(lengths.type())
    delta /= torch.max(lengths, eps)
